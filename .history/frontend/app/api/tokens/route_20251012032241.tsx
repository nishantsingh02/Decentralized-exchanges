...
export async function GET(req: NextRequest) {
    ...
    const balances = await Promise.all(supportedTokens.map(s => getAccountBalance(s, address)))
    

    return NextResponse.json({
    tokens: supportedTokens.map((item, index) => ({ // <-- Clear: key is 'tokens', variable is 'item'
        ...item,
